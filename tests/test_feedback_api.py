...
def test_submit_feedback_rejects_read_only_role(monkeypatch, tmp_path):
    ...
    assert response.json()["detail"] == "Feedback submission requires planner, maintainer, or admin role"
...
def test_list_feedback_requires_run_or_recommendation_filter(monkeypatch):
    ...
    response = client.get("/api/v1/rca/feedback", headers={"x-user-id": "viewer-1", "x-user-role": "viewer"})
