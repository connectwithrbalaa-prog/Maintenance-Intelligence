...
def test_servicenow_smoke_approval_flow(httpserver, monkeypatch, tmp_path):
    summaries = tmp_path / "outputs"
    _write_summary(summaries)
    fake_connection = FakeConnection()

    httpserver.expect_request("/api/now/table/wm_order", method="POST").respond_with_json(
        {
            "result": {
                "number": "WO0001234",
                "state_display": "Open",
    ...
    monkeypatch.setenv("MI_SERVICENOW_BASE_URL", httpserver.url_for(""))
    monkeypatch.setenv("MI_SERVICENOW_TABLE", "wm_order")
    ...
    assert payload["work_order"]["wo_id"] == "WO0001234"
    assert payload["work_order"]["backend"] == "servicenow"
    assert payload["work_order"]["lifecycle_phase"] == "active"
