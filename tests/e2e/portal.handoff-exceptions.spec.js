...
test('portal handoff exceptions: current rank chip updates with queue view', async ({ page }) => {
  const harness = createPortalHarness();
  await harness.install(page);
  ...
  await expect(summaryRow.getByText('Current rank #3', { exact: true })).toBeVisible();

  await page.getByLabel('Handoff queue view').selectOption('connector-failure');
  await expect(summaryRow.getByText('Current rank clear', { exact: true })).toBeVisible();
  await expect(summaryRow.getByText('Current rank #3', { exact: true })).not.toBeVisible();
});

test('portal handoff exceptions: aging risk chip updates with queue view', async ({ page }) => {
  const harness = createPortalHarness();
  await harness.install(page);
  ...
  await expect(summaryRow.getByText('Aging risk 3', { exact: true })).toBeVisible();

  await page.getByLabel('Handoff queue view').selectOption('connector-failure');
  await expect(summaryRow.getByText('Aging risk 1', { exact: true })).toBeVisible();
