...
def test_build_early_warning_report_ranks_assets_by_persistent_signal_pressure():
    ...
    report = build_early_warning_report(
        [
            ("PUMP-202", "high", datetime(2026, 3, 18, 10, 0, tzinfo=timezone.utc), {"temperature": 92}),
        ...
        [
            ("PUMP-202", "vibration", "1h", datetime(2026, 3, 18, 10, 0, tzinfo=timezone.utc), 9.8, 10.7, {"high_vibration": True, "z_score_spike": True}),
            ("PUMP-202", "temperature", "6h", datetime(2026, 3, 18, 9, 0, tzinfo=timezone.utc), 88.0, 91.0, {"high_temperature": True}),
            ("PUMP-101", "vibration", "24h", datetime(2026, 3, 16, 8, 0, tzinfo=timezone.utc), 6.4, 6.9, {}),
        ],
        now=now,
    )
    ...
    assert pump_101["early_warning_reasons"] == ["Low-volume warning signals are present but not yet persistent"]
...
def test_build_early_warning_report_keeps_rollup_only_assets_and_emits_watch_status():
    ...
    report = build_early_warning_report(
        [],
        [
            ("FAN-9", "temperature", "1h", datetime(2026, 3, 18, 11, 0, tzinfo=timezone.utc), 84.0, 86.0, {"high_temperature": True}),
            ("FAN-9", "temperature", "6h", datetime(2026, 3, 18, 10, 0, tzinfo=timezone.utc), 82.0, 84.0, {"high_temperature": True}),
            ("FAN-9", "temperature", "24h", datetime(2026, 3, 18, 9, 0, tzinfo=timezone.utc), 80.0, 82.0, {"high_temperature": True}),
        ],
        now=now,
    )

    assert report["summary"]["status_counts"] == {"critical": 0, "elevated": 0, "watch": 1, "normal": 0}
