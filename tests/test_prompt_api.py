...
def test_prompt_api_list_and_admin_update(monkeypatch):
    ...
    monkeypatch.setattr(
        prompts_api,
        "list_prompts",
        lambda conn, route_name=None: [
            {"prompt_id": "rca-default-v1", "route_name": route_name or "rca"}
        ],
    )
    monkeypatch.setattr(
        prompts_api,
        "load_route_config",
        lambda conn, settings, route_name, org_id: {
    ...
    monkeypatch.setattr(
        prompts_api,
        "save_route_config",
        lambda conn, route_name, org_id, payload: {
            "route_name": route_name,
            "org_id": org_id,
            **payload,
        },
...
def test_feedback_prompt_metric_increments(monkeypatch):
    monkeypatch.setenv("MI_AUTH_MODE", "api_key")
    monkeypatch.setenv(
        "MI_API_KEYS", json.dumps({"operator-key": {"org_id": "ORG-1", "role": "operator"}})
    )
    ...
    client = TestClient(app)
    before = prompt_feedback_total.labels(
        route="rca", prompt_id="rca-default-v1", action="accept"
    )._value.get()
    ...
    after = prompt_feedback_total.labels(
        route="rca", prompt_id="rca-default-v1", action="accept"
    )._value.get()
