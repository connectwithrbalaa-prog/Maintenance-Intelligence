...
def test_call_rca_returns_tokens_and_parsed_structure(monkeypatch):
    class FakeResponse:
        def __init__(self):
            self.choices = [SimpleNamespace(message=SimpleNamespace(content='{"title":"Bearing issue","confidence":0.9}'))]
...
def test_call_rca_returns_error_text_and_fallback_structure_on_client_failure(monkeypatch):
    ...
    class FakeClient:
        def __init__(self, api_key):
            self.chat = SimpleNamespace(
                completions=SimpleNamespace(
                    create=lambda **kwargs: (_ for _ in ()).throw(RuntimeError("timeout talking to model"))
                )
            )
