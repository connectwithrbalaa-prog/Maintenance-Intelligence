...
def test_health_deep_reports_ok_when_pg_kafka_and_lag_are_healthy(monkeypatch):
    ...
    monkeypatch.setattr(
        health_mod, "Settings", lambda: SimpleNamespace(pg_dsn="dsn", kafka_bootstrap="kafka:9092")
    )
    ...
    monkeypatch.setattr(
        health_mod, "compute_kafka_lag", lambda *args, **kwargs: {"_summary": {"total_lag": 12}}
    )
...
def test_health_deep_degrades_when_dependencies_fail_or_lag_is_unknown(monkeypatch):
    ...
    monkeypatch.setattr(
        health_mod.psycopg2, "connect", lambda dsn: (_ for _ in ()).throw(RuntimeError("pg down"))
    )
    monkeypatch.setattr(
        health_mod,
        "KafkaAdminClient",
        lambda **kwargs: (_ for _ in ()).throw(RuntimeError("kafka down")),
    )
    monkeypatch.setattr(
        health_mod, "compute_kafka_lag", lambda *args, **kwargs: {"_summary": {"total_lag": None}}
    )
