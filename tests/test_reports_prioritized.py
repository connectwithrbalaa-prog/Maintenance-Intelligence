...
from maintenance_intelligence.services import pdm_scorer as pdm_scorer_mod
...
class FakePrioritizedCursor:
    ...
    def execute(self, sql):
        ...
        if (
            ...
            return
        if (
            "SELECT DISTINCT ON (asset_id) asset_id, severity, occurred_at FROM events"
            in normalized
        ):
            self.rows = [
            ...
            return
        if (
            "SELECT asset_id, COUNT(*) AS wo_count, SUM(CASE WHEN UPPER(COALESCE(status, '')) IN ('COMP', 'COMPLETE', 'COMPLETED', 'CLOSE', 'CLOSED', 'DONE') THEN 0 ELSE 1 END) AS open_wo_count FROM workorders"
            in normalized
        ):
            self.rows = [
                ("PUMP-101", 1, 0),
            ...
            return
        if (
            "SELECT asset_id, SUM(CASE WHEN action = 'accept' THEN 1 ELSE 0 END) AS accept_count, SUM(CASE WHEN action = 'reject' THEN 1 ELSE 0 END) AS reject_count FROM rca_feedback"
            in normalized
        ):
            self.rows = [
                ("PUMP-101", 2, 0),
            ...
            return
        if (
            "SELECT asset_id, signal_type, period, end_time, mean, max, anomaly_flags FROM signal_rollups"
            in normalized
        ):
            self.rows = [
                (
                ...
                (
                    ...
                    91.0,
                    94.0,
                    {"high_temperature": True, "z_score_spike": True},
                ),
            ]
            return
        if "SELECT asset_id, severity, occurred_at, details FROM events" in normalized:
            self.rows = [
                (
                    ...
                    {"rms": 8.1, "threshold": 9.0},
                ),
                (
                    "PUMP-101",
                    "high",
                    datetime(2026, 3, 12, 8, 0, tzinfo=timezone.utc),
                    {"rms": 7.9, "threshold": 9.0},
                ),
                (
                    "PUMP-101",
                    "high",
                    datetime(2026, 3, 15, 10, 0, tzinfo=timezone.utc),
                    {"rms": 8.0, "threshold": 9.0},
                ),
                (
                    "PUMP-202",
                    "medium",
                    datetime(2026, 3, 15, 7, 0, tzinfo=timezone.utc),
                    {"temperature": 92.0, "threshold": 90.0},
                ),
                (
                    "PUMP-202",
                    "medium",
                    datetime(2026, 3, 15, 11, 0, tzinfo=timezone.utc),
                    {"temperature": 93.0, "threshold": 90.0},
                ),
            ]
            return
...
def test_prioritized_assets_requires_authenticated_identity(monkeypatch):
    ...
    assert (
        response.json()["detail"] == "Prioritized asset reports require an authenticated identity"
    )
...
def test_prioritized_assets_ranks_assets_by_signal_feedback_and_mtbf_risk(monkeypatch):
    ...
    # Fix "now" to 2026-03-15 12:00 UTC so events from that day are "fresh" (< 24h)
    _fixed_now = datetime(2026, 3, 15, 12, 0, tzinfo=timezone.utc)
    monkeypatch.setattr(
        reports_mod,
        "build_early_warning_report",
        partial(pdm_scorer_mod.build_early_warning_report, now=_fixed_now),
    )

    client = TestClient(app)
    response = client.get(
        "/api/v1/reports/prioritized-assets?limit=5&window=30", headers=READ_HEADERS
    )
    ...
    assert second["early_warning_score"] == 63.0
...
def test_prioritized_assets_filters_to_warning_status_assets(monkeypatch):
    ...
    client = TestClient(app)
    response = client.get(
        "/api/v1/reports/prioritized-assets?limit=5&window=30&warnings_only=true",
