...
def test_run_falls_back_to_sql_migrations_when_alembic_missing(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        migrate_mod.logger, "warning", lambda payload: warning_events.append(payload)
    )
    monkeypatch.setattr(migrate_mod.logger, "info", lambda payload: info_events.append(payload))
    monkeypatch.setitem(
        sys.modules,
        "glob",
        SimpleNamespace(glob=lambda pattern: [str(first_file), str(second_file)]),
    )
    ...
    assert any(
        "CREATE TABLE IF NOT EXISTS mi_schema_migrations" in query for query in executed_queries
    )
    ...
    assert {event["event"] for event in info_events} == {
        "migration.skip",
        "migration.apply",
        "migration.done",
    }
