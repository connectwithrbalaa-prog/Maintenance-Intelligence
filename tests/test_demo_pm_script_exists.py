...
def test_demo_pm_script_can_use_existing_api(tmp_path) -> None:
    ...
    class DemoHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            if self.path == "/api/v1/agents/pm/advisor/analyze":
                self._write_json({"proposal": {"proposal_id": "demo-proposal"}})
                return
            if self.path == "/api/v1/agents/pm/proposals/demo-proposal/approve":
                self._write_json({"approved": True, "proposal_id": "demo-proposal"})
                return
            self.send_error(404)
    ...
    try:
        ...
        env["MI_RUN_SUMMARY_DIR"] = str(tmp_path / "outputs")
    ...
    assert "Created proposal:" in result.stdout
