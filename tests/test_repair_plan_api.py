...
def test_repair_plan_endpoints_round_trip(monkeypatch):
    ...
    monkeypatch.setattr(repair_plan_mod, "create_repair_plan", lambda dsn, **kwargs: {**fake.created, **kwargs})
    monkeypatch.setattr(repair_plan_mod, "get_repair_plan", lambda dsn, plan_id: fake.created if plan_id == "RP-123" else None)
    monkeypatch.setattr(repair_plan_mod, "list_repair_plans", lambda dsn, limit=100: [fake.created])
    monkeypatch.setattr(repair_plan_mod, "delete_repair_plan", lambda dsn, plan_id: plan_id == "RP-123")
    monkeypatch.setattr(repair_plan_mod, "add_part_to_plan", lambda dsn, plan_id, **kwargs: {**fake.part, **kwargs, "plan_id": plan_id})
    monkeypatch.setattr(repair_plan_mod, "list_parts_for_plan", lambda dsn, plan_id: [fake.part] if plan_id == "RP-123" else [])
    ...
    delete_response = client.delete("/api/v1/repair-plans/RP-123", headers={"x-user-id": "admin-1", "x-user-role": "admin"})
...
def test_repair_plan_openapi_schema_is_exposed():
    ...
    assert payload["paths"]["/api/v1/repair-plans/{plan_id}/parts"]["post"]["summary"] == "Add a repair part"
...
def test_repair_plan_write_endpoints_require_allowed_role(monkeypatch):
    ...
    assert create_response.json()["detail"] == "Repair plan writes require planner, maintainer, or admin role"
    ...
    assert add_part_response.json()["detail"] == "Repair plan writes require planner, maintainer, or admin role"
    ...
    assert delete_response.json()["detail"] == "Repair plan writes require planner, maintainer, or admin role"
