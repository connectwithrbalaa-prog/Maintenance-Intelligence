...
def _validate_lookup_id(value: str, field_name: str) -> str:
    ...
    if not candidate or candidate != value or len(candidate) > 255 or any(ord(char) < 32 for char in candidate):
...
def _feedback_defaults(payload: Optional[Dict[str, Any]]) -> Dict[str, Optional[str]]:
    ...
    context_meta = payload.get("context_meta") if isinstance(payload.get("context_meta"), dict) else {}
    return {
        "recommendation_id": _as_text(payload.get("recommendation_id")) or _as_text(payload.get("run_id")),
...
def _authorize_feedback_submission(request: Request, claimed_user_id: Optional[str]) -> str:
    ...
    if actor_id is None:
        raise HTTPException(status_code=403, detail="Feedback submission requires an authenticated identity")
    if actor_role not in APPROVAL_ROLES:
        raise HTTPException(status_code=403, detail="Feedback submission requires planner, maintainer, or admin role")
    claimed_actor = _as_text(claimed_user_id)
    if claimed_actor is not None and claimed_actor != actor_id:
        raise HTTPException(status_code=403, detail="Feedback actor must match authenticated identity")
    return actor_id
...
def list_feedback(
    ...
    require_authenticated_identity(request, detail="Feedback history requires an authenticated identity")
    filters = []
    params: List[Any] = []
