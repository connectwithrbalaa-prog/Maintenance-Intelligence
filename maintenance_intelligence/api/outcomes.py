...
def _proposal_failure_summary(metadata: Any) -> Dict[str, Any]:
    ...
    failure_summary = (
        approval.get("failure_summary") if isinstance(approval.get("failure_summary"), dict) else {}
    )
    if failure_summary:
...
def _update_cmms_bucket(
    ...
    if handoff_state == "success":
        ...
        if bool(failure_summary.get("retryable")):
            summary["retryable_failure_total"] = (
                int(summary.get("retryable_failure_total") or 0) + 1
            )
        else:
...
def _update_backend_metrics(out: Dict[str, Any], rows: List[Any], bucket_dates: List[str]) -> None:
    ...
    for row in rows or []:
        ...
        if handoff_state in {"success", "failure"}:
            ...
            if handoff_state == "success":
                ...
                retryable_failure_counts[backend_key] = (
                    int(retryable_failure_counts.get(backend_key) or 0) + 1
                )
            else:
                terminal_failure_counts[backend_key] = (
                    int(terminal_failure_counts.get(backend_key) or 0) + 1
                )
    ...
    for backend_key in set(backend_metrics) | set(total_counts) | set(terminal_counts):
        ...
        backend_entry["retryable_failure_total"] = int(
            retryable_failure_counts.get(backend_key) or 0
        )
...
def rca_outcomes_csv(request: Request, window: int = Query(30, ge=1, le=365)):
    ...
    rows.append(
        {
            "metric": "cmms_retryable_failure_total",
            "value": cmms_summary.get("retryable_failure_total", 0),
        }
    )
    rows.append(
        {
            "metric": "cmms_terminal_failure_total",
            "value": cmms_summary.get("terminal_failure_total", 0),
        }
    )
    rows.append(
        {
