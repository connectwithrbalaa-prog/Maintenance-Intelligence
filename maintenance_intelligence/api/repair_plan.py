...
from maintenance_intelligence.services.repair_plan_service import (
    create_repair_plan, get_repair_plan, list_repair_plans, delete_repair_plan,
    add_part_to_plan, list_parts_for_plan
)

router = APIRouter(prefix="/api/v1/repair-plans", tags=["repair-plan"])

class RepairPartSchema(BaseModel):
    part_id: Optional[str] = Field(default=None, description="Server-assigned repair part identifier")
    ...
    created_at: Optional[str] = Field(default=None, description="Creation timestamp in ISO-8601 format")

class RepairPlanSchema(BaseModel):
    plan_id: Optional[str] = Field(default=None, description="Server-assigned repair plan identifier")
    run_id: Optional[str] = Field(default=None, description="RCA run identifier linked to this repair plan")
    recommendation_id: Optional[str] = Field(default=None, description="Recommendation identifier linked to this repair plan")
    ...
    confidence: Optional[float] = Field(default=None, description="Model confidence between 0 and 1", ge=0, le=1)
    status: Optional[str] = Field(default=None, description="Current repair plan status")
    created_at: Optional[str] = Field(default=None, description="Creation timestamp in ISO-8601 format")
    updated_at: Optional[str] = Field(default=None, description="Last update timestamp in ISO-8601 format")


class RepairPlanDetailSchema(RepairPlanSchema):
    parts: list[RepairPartSchema] = Field(default_factory=list, description="Parts attached to this repair plan")
...
def _require_write_access(request: Request) -> str:
    ...
    if actor_id is None:
        raise HTTPException(status_code=403, detail="Repair plan writes require an authenticated identity")
    if actor_role not in APPROVAL_ROLES:
        raise HTTPException(status_code=403, detail="Repair plan writes require planner, maintainer, or admin role")
    return actor_id


def _require_read_access(request: Request) -> None:
    require_authenticated_identity(request, detail="Repair plan reads require an authenticated identity")
