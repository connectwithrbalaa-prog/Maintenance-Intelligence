...
class HybridRetriever:
    ...
    def _vector_search(
        self, query: str, asset_id: Optional[str], limit: int, fleet_wide: bool = False
    ) -> List[Dict[str, Any]]:
    ...
    def _bm25_search(
        ...
        with conn, conn.cursor() as cur:
            ...
            if asset_id and not fleet_wide:
                cur.execute(
                    "SELECT chunk_id, title, content, asset_id, source FROM doc_chunks WHERE asset_id = %s",
                    (asset_id,),
                )
            else:
                ...
                scored.append(
                    {
                        ...
                        "asset_class": None,
                        "source_scope": (
                            "local" if not asset_id or row_asset_id == asset_id else "fleet"
                        ),
    ...
    def _bm25_score(self, query_terms: List[str], doc_text: str, all_docs: List[Tuple]) -> float:
        ...
        avg_doc_len = sum(
            len(self._tokenize(f"{title} {content}")) for _, title, content, *_ in all_docs
        ) / len(all_docs)
        ...
        for term in query_terms:
            if term in doc_term_freq:
                ...
                df = sum(
                    1
                    for _, title, content, *_ in all_docs
