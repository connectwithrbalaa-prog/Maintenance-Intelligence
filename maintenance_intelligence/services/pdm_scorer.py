...
def _update_event_state(
    asset: Dict[str, Any], severity: Any, occurred_at: Any, details: Any
) -> None:
    ...
    asset["max_severity_points"] = max(
        float(asset.get("max_severity_points") or 0.0), severity_points
    )
...
def _update_rollup_state(
    ...
    if flag_count > 0:
        ...
        asset["anomaly_flag_points"] = float(asset.get("anomaly_flag_points") or 0.0) + (
            _ROLLUP_PERIOD_POINTS.get(period_key, 2.0) * flag_count
        )
    ...
    if signal_key == "vibration":
        ...
        asset["rollup_temperature_peak"] = _max_value(
            asset.get("rollup_temperature_peak"), peak_value
        )
...
def _score_asset(asset: Dict[str, Any], now: datetime) -> Tuple[float, str, List[str]]:
    ...
    if vibration_peak is not None:
        if vibration_peak >= 10.0:
            ...
            score += 10.0
            reasons.append(
                _reason_value("Vibration is trending high at", vibration_peak, " mm/s") or ""
            )

    temperature_peak = _max_value(
        asset.get("temperature_peak"), asset.get("rollup_temperature_peak")
    )
    if temperature_peak is not None:
        if temperature_peak >= 90.0:
            ...
            score += 10.0
            reasons.append(
                _reason_value("Temperature is trending high at", temperature_peak, " C") or ""
            )
